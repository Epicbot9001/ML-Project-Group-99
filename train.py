...
model.add(Dense(8, activation='relu', kernel_regularizer=l2(0.005)))
model.add(Dense(1, activation='linear'))
model.compile(optimizer=Adam(learning_rate=0.0005), loss='mean_squared_error', metrics=["mae", custom_metric])
# Train the model and save the history
history = model.fit(x, y, epochs=20, batch_size=1684, validation_split=0.1)
...
# Plot mae metric
fig_mae.add_trace(go.Scatter(x=np.arange(1, len(history.history['mae']) + 1), y=history.history['mae'], mode='lines', name='MAE'))

# Plot validation mae metric if available
if 'val_mae' in history.history:
    fig_mae.add_trace(go.Scatter(x=np.arange(1, len(history.history['val_mae']) + 1), y=history.history['val_mae'], mode='lines', name='MAE Loss'))
